...
function setupRoutes(router: Router) {
  router.beforeEach(async (to, _from, next) => {
    ...
    // 是否已登录（修复：isLogin 为 ref，需解包）
    if (unref(userStore.isLogin)) {
      // 若为客服角色：切换为仅客服可见的路由集合
      const isAgent = (userStore as any).role === 'agent' || localStorage.getItem('role') === 'agent'
      if (isAgent && !routeStore.isGenerate) {
        // 仅保留客服中心的异步路由
        const onlyCS: any[] = (asyncRoutes as any).filter((g: any) => {
          // 分组里包含 path '/cs' 的那一组
          return g?.children?.some?.((c: any) => c.path === '/cs')
        }).map((g: any) => ({
          meta: g.meta,
          children: g.children.filter((c: any) => c.path === '/cs'),
        }))
        routeStore.generateRoutesAtFront(onlyCS as any)
      }
      // 是否已根据权限动态生成并注册路由
        ...
        try {
          ...
          if (!isAgent) {
            ...
            switch (settingsStore.settings.app.routeBaseOn) {
              case 'frontend':
                routeStore.generateRoutesAtFront(asyncRoutes as any)
                break
              case 'backend':
