...
export const useMenuStore = defineStore(
  ...
  () => {
    ...
    function getDefaultOpenedPaths(menus: Menu.recordRaw[], rootPath = '') {
      const defaultOpenedPaths: string[] = []
      menus.forEach((item) => {
        // 自动展开所有有子菜单的项，不再依赖 defaultOpened 设置
        if (item.children && item.children.length > 0) {
          const currentPath = resolveRoutePath(rootPath, item.path)
          defaultOpenedPaths.push(currentPath)
          const childrenDefaultOpenedPaths = getDefaultOpenedPaths(item.children, currentPath)
